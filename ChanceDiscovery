...
def tag_language(corpus):
    corpus['language'] = corpus['content'].apply(lambda x: langid.classify(x)[0])
    return corpus
...
def show_keygraph(keygraph, corpus_file, M, K):
    ...
    with st.container():
        components.html(open(f'C:\\Users\\user\\Downloads\\chance-discovery\\graphs\\{'customer-support_25_12'}.html', 'r',
...
if corpus_file is not None:
    ...
    # Remove @mentions
    corpus['content'] = corpus['content'].apply(
        lambda x: ' '.join([word for word in x.split() if not word.startswith('@')]))

    # corpus contains content in multiple languages
    corpus = tag_language(corpus)

    st.subheader("Corpus")
    show_corpus(corpus)

    # remove content that is not in English
